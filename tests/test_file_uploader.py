...
from mantarray_desktop_app.file_uploader import get_analysis_status
...
def test_get_access_token__requests_and_returns_access_token_correctly(mocker):
    ...
    actual = get_access_token(TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)
    mocked_post.assert_called_once_with(
        f"https://{CLOUD_API_ENDPOINT}/users/login",
        json={"customer_id": TEST_CUSTOMER_ID, "username": TEST_USER_ID, "password": TEST_PASSWORD},
    )
...
def test_get_analysis_status__requests_and_returns_analysis_status_correctly(mocker):
    ...
    actual = get_analysis_status(test_access_token, test_id)
...
def test_get_analysis_status__raises_error_if_analysis_job_errored(mocker):
    ...
    with pytest.raises(CloudAnalysisJobFailedError, match=expected_error_msg):
        get_analysis_status("access_token", "job_id")
...
def test_uploader__does_not_create_new_zip_file_if_file_to_upload_is_a_zip_file(mocker):
    ...
    mocker.patch.object(file_uploader, "get_analysis_status", autospec=True)

    mocked_create_zip_file = mocker.patch.object(file_uploader, "create_zip_file", autospec=True)

    uploader(TEST_FILEPATH, TEST_FILENAME, TEST_ZIPDIR, TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)
    mocked_create_zip_file.assert_not_called()


def test_uploader__sleeps_in_between_polling_analysis_status_until_analysis_completes(mocker):
    mocker.patch.object(file_uploader, "get_access_token", autospec=True)
    mocker.patch.object(file_uploader, "get_file_md5", autospec=True)
    mocker.patch.object(file_uploader, "get_upload_details", autospec=True)
    mocker.patch.object(file_uploader, "upload_file_to_s3", autospec=True)
    ...
    mocked_get_analysis_status = mocker.patch.object(
        file_uploader, "get_analysis_status", autospec=True, side_effect=test_status_dicts
    )
...
def test_uploader__runs_upload_procedure_correctly_for_recording(user_dir_exists, mocker):
    ...
    mocked_get_analysis_status = mocker.patch.object(file_uploader, "get_analysis_status", autospec=True)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        zipped_file_name = f"{tmp_dir}.zip"
        zipped_file_path = mocked_create_zip_file.return_value

        uploader(TEST_FILEPATH, tmp_dir, TEST_ZIPDIR, TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)
    ...
    mocked_get_access_token.assert_called_once_with(TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)
    ...
    mocked_upload_file.assert_called_once_with(zipped_file_path, zipped_file_name, expected_upload_details)
    mocked_start_analysis.assert_called_once_with(expected_access_token, expected_upload_details["id"])
    mocked_get_analysis_status.assert_called_once_with(
        expected_access_token, mocked_start_analysis.return_value
    )
    mocked_download_analaysis.assert_called_once_with(
        mocked_get_analysis_status.return_value["url"], zipped_file_name
    )
...
def test_uploader__runs_upload_procedure_correctly_for_log_files(mocker):
    ...
    spied_get_analysis_status = mocker.spy(file_uploader, "get_analysis_status")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        uploader(TEST_LOGPATH, tmp_dir, TEST_ZIPDIR, TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)

    mocked_create_zip_file.assert_called_once_with(TEST_LOGPATH, tmp_dir, TEST_ZIPDIR)
    mocked_get_access_token.assert_called_once_with(TEST_CUSTOMER_ID, TEST_USER_ID, TEST_PASSWORD)
    mocked_get_file_md5.assert_called_once_with(zipped_file_path)
    mocked_get_upload_details.assert_called_once_with(
        expected_access_token, zipped_file_name, TEST_CUSTOMER_ID, expected_md5, LOG_UPLOAD_TYPE
    )
    mocked_upload_file.assert_called_once_with(zipped_file_path, zipped_file_name, expected_upload_details)
