...
def test_chunk_subprotocol__handles_nested_loops():
    test_pulses = [
        get_random_stim_pulse(
            total_subprotocol_dur_us=int(STIM_MAX_CHUNKED_SUBPROTOCOL_DUR_MICROSECONDS * 2.5), freq=1
        ),
                    ...
                    "subprotocols": [
                        ...
                        (
                            None,
                            {
                                "type": "loop",
                                "num_iterations": test_num_loop_iterations[1],
                                "subprotocols": [chunk_subprotocol(test_pulses[2])],
                            },
                        ),
                        chunk_subprotocol(test_pulses[3]),
                    ],
                },
            ),
        ],
    }
    assert chunk_subprotocol(test_protocol) == (None, expected_chunked_subprotocol)
