...
def test_MantarrayMCSimulator__puts_status_beacon_into_output_queue_on_initialization(
    mantarray_mc_simulator,
):
    ...
    # TODO make constant
    assert "CURI BIO" in str(init_status_beacon)
...
def test_MantarrayMCSimulator__puts_status_beacon_into_output_queue_every_5_seconds__and_includes_correct_timestamp(
    ...
    expected_durs = [
        CENTIMILLISECONDS_PER_SECOND * 5,
        CENTIMILLISECONDS_PER_SECOND * 10,
    ]
    mocker.patch.object(
    ...
    # 1 second since last beacon
    invoke_process_run_and_check_errors(simulator)
    confirm_queue_is_eventually_empty(output_queue)
    # 5 seconds since last beacon
    invoke_process_run_and_check_errors(simulator)
    confirm_queue_is_eventually_of_size(output_queue, 1)
    expected_handshake = (
        b"CURI BIO" + expected_durs[0].to_bytes(8, "little") + b"\x00\x04"
    )
    assert output_queue.get(timeout=QUEUE_CHECK_TIMEOUT_SECONDS) == expected_handshake
    # 4 seconds since last beacon
    invoke_process_run_and_check_errors(simulator)
    confirm_queue_is_eventually_empty(output_queue)
    # 6 seconds since last beacon
    invoke_process_run_and_check_errors(simulator)
    confirm_queue_is_eventually_of_size(output_queue, 1)
    expected_handshake = (
        b"CURI BIO" + expected_durs[1].to_bytes(8, "little") + b"\x00\x04"
    )
    assert output_queue.get(timeout=QUEUE_CHECK_TIMEOUT_SECONDS) == expected_handshake
