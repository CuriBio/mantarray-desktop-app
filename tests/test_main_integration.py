...
LIVE_VIEW_ACTIVE_WAIT_TIME = 60
CALIBRATED_WAIT_TIME = 5
STOP_MANAGED_ACQUISITION_WAIT_TIME = 20
INTEGRATION_TEST_TIMEOUT = 160
...
def test_system_states_and_recording_files_with_file_directory_passed_in_cmd_line_args(
    patched_xem_scripts_folder,
    patched_firmware_folder,
    test_process_manager_without_created_processes,
    test_client,
    fully_running_app_from_main_entrypoint,
    mocker,
):
    ...
    with tempfile.TemporaryDirectory() as expected_recordings_dir:
        ...
        _ = fully_running_app_from_main_entrypoint(command_line_args)
        wait_for_subprocesses_to_start()
        response = requests.get(f"{get_api_endpoint()}system_status")
        assert response.status_code == 200
        assert response.json()["ui_status_code"] == str(
        ...
        test_process_manager_without_created_processes.soft_stop_processes()

        fw_process = (
            test_process_manager_without_created_processes.get_file_writer_process()
        )
        fw_process.close_all_files()
        actual_set_of_files = set(
            os.listdir(
        ...
        test_process_manager_without_created_processes.hard_stop_and_join_processes()
